...
<div class="form-container">
    <div class="form-card">
        ...
        <!-- Status Badge -->
        <div>
            <span class="status-badge @if ($kasKeluar->status === 'pending') status-pending @elseif ($kasKeluar->status === 'approved') status-approved @else status-rejected @endif">
                <i class="bi @if ($kasKeluar->status === 'pending') bi-clock @elseif ($kasKeluar->status === 'approved') bi-check-circle @else bi-x-circle @endif"></i>
                Status: {{ ucfirst($kasKeluar->status) }}
            </span>
        </div>
        ...
        <form action="{{ route('kas_keluar.update', $kasKeluar) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input 
                    ...
                    id="tanggal" 
                    name="tanggal" 
                    value="{{ old('tanggal', $kasKeluar->tanggal->format('Y-m-d')) }}" 
                    required
                >
                @error('tanggal')
            ...
            <div class="form-group">
                ...
                <textarea 
                    ...
                    rows="4"
                    required
                >{{ old('keterangan', $kasKeluar->keterangan) }}</textarea>
                @error('keterangan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
